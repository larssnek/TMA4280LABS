...
int main(int argc, char **argv)
{
	...
	for (size_t i = 0; i < nrows[rank]; i++) {
		for (size_t j = 0; j < m; j++) {
			u_exact=sin(PI*grid[local_displacement[rank]+i+1])*sin(2*PI*grid[j+1]);
			error=fabs(u_exact-b[i][j]);
	...
	if(rank==0){
		double duration = (omp_get_wtime() - start);
            	printf("run time: %e\n", duration);	
		printf("global_max_error = %e\n", global_max_error);
		FILE *f = fopen("maxerror.txt", "a");
		if (f == NULL)
		{
